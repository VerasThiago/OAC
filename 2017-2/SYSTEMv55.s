...
.eqv LINHA1             0xFF100130
.eqv LINHA2             0xFF100140
.eqv LCDClear           0xFF100150
.eqv AudioINL           0xFF100000
.eqv AudioINR           0xFF100004
.eqv AudioOUTL          0xFF100008
.eqv AudioOUTR          0xFF10000c
.eqv AudioCTRL1         0xFF100010
.eqv AudioCTRL2         0xFF100014
.eqv SD_BUFFER_INI      0xFF100250
.eqv SD_BUFFER_END      0xFF10044C
.eqv SD_INTERFACE_ADDR  0xFF100450
.eqv SD_INTERFACE_CTRL  0xFF100454
.eqv STOPWATCH		0xFF10050C
.eqv LFSR		0xFF100510

# Sintetizador - 2015/1
.eqv NoteData           0xFF100200
.eqv NoteClock          0xFF100204
.eqv NoteMelody         0xFF100208
.eqv MusicTempo         0xFF10020C
.eqv MusicAddress       0xFF100210
...
# KEYBOARD and DISPLAY MMIO DISPLAY TOOL
KEYBOARD_BUFFER: .word 0
...
exceptionHandling:  addi    $sp, $sp, -8 # inicio do exception handler
    sw      $at, 0($sp)
    sw      $ra, 4($sp)
...
interruptException:     mfc0    $k0, $13  # le o codigo da interrupcao
    ...
    andi    $k1, $k0, 0x0008            #verificar se nao  seria 0x0008     NOTE: Mas estÃ¡ 0x0008!
    bne     $k1, $zero, counterInterrupt

	andi $k1, $k0, 0x0010               # Keyboard and Display Interrupt Tool
	bne $k1, $zero, KDInterrupt
	
    j       endException

KDInterrupt:	la $k1,0xFF100000
		lw $k0,4($k1)	            # codigo ascii da tecla lida
		la $k1,KEYBOARD_BUFFER
		sw $k0,0($k1)
		#sw $k0,12($k1)	            # imprime no Display <<<<<<<<<<<<<< Colocar aqui o que deve ser feito com a tecla lida
		
		mfc0 $k0,$14  	            # O Mars ja soma 4! 
		addi $k0,$k0,-4	            # Então precisa tirar 4 do EPC
		mtc0 $k0,$14 
		j endException
...
instructionExceptionLoop:     beq     $t1, $zero, instructionExceptionVGA        # fim da string
    sb      $t1, 0($t9)
    addi    $t0, $t0, 1
    addi    $t9, $t9, 1
    lb      $t1, 0($t0)
    j       instructionExceptionLoop
    # mostra mensagem de erro no VGA
  instructionExceptionVGA: la $a0, instructionMessage
  	li $a1,0
  	li $a2,0
  	li $a3,0x0F
  	jal printString
...
endSyscall:     lw      $1,     0($sp)  # recupera todos os registradores na pilha
...
goToExit1:     la      $t9, LINHA1                 # escreve FIM no LCD  <= RETIREI mudar o goToExit1  NOTE: nao entendi
    nop
    nop
...
goToExit:     j       goToExit                    ########### syscall 10
...
goToPrintFloat:     la      $s6, FloatBuffer            # add $s6, $zero, $zero        #chama float e printstring
    jal     printFloat
    la      $a0, FloatBuffer
    jal     printString
    j       endSyscall
...
printZero:     addi    $a0, $zero, 48              # Imprime 0
    jal     printChar
...
PrintIntContinue:     beq     $t0, $zero, PrintIntPop     # se $t0 e zero, nao ha¡ mais digitos para imprimir
    ...
    addi    $t3, $t3, 1                 # conta quantos elementos (digitos) estÃ£o na pilha
...
PrintIntPop:     beq     $t3, $zero, printIntEnd     # ultimo digito endPrintInt
    ...
    addi    $sp, $sp, 4
    nop                                 # hazard lw addi
    addi    $a0, $a0, 48                # codigo ASCII do digito = numero + 48
    nop
...
continua:     andi    $t4, $t2, 0xFF              # ultima tecla inserida
    nop
    nop
    beq     $t4, $t7, shiftindahouse    # se valor e shift
...
shiftindahouse:     addi    $t9, $zero, 1               # //NOTE: Havia uma virgula apos addi. Nao sei como o montador interpretava aquilo...
...
continue:     addi    $sp, $sp, -4
    ...
    mtc1    $t0, $f15
sePositivo:     add.s   $f10, $f10, $f15
    j       continue100
seNegativo:     sub.s   $f10, $f10, $f15

continue100:     neg     $t8,$t8
    and     $t6, $t9, $t8
    bnez    $t6, NAN
...
CONTINUE1:     addiu   $t0, $zero, '+'
    or      $s5, $t0, $zero             #first byte saved
#------------
# Imprimir numero antes do ponto

#Verificar se a mantissa esta normalizada
CONTINUE2:     seq     $t8, $s1, 1
    beqz    $t8, normalizaPraCima
    li      $t0, 0xBF800000             # -1
    mtc1    $t0, $f25                   # -1.0
    mul.s   $f10, $f10, $f25            # turns it positive

normalizaPraCima:     li      $t0, 0x3F800000             # 1
    mtc1    $t0, $f23
    li      $t0, 0x41200000             # 10
    mtc1    $t0, $f24
    c.lt.s  $f10, $f23                  # ve se mantissa e menor que 1
    bc1f    NOTZERO
    mul.s   $f10, $f24, $f10
    addi    $s4, $s4, -1                # decrementa o expoente por 1
    j       normalizaPraCima

normalizaPraBaixo:     li      $t0, 0x41200000             # 10
    mtc1    $t0, $f24
    c.lt.s  $f10, $f24                  # ve se mantissa e menor que 1
    bc1t    NOTZERO
    div.s   $f10, $f10, $f24
    addi    $s4, $s4, +1                # decrementa o expoente por 1
    j       normalizaPraBaixo

NOTZERO:     li      $t0, 0x41200000             # 10
    mtc1    $t0, $f14
    c.lt.s  $f10, $f14
    bc1t    CONTINUE3
...
a1:     addi    $t0, $t0, '0'               # Inicio dos caracteres de numeros, 48 = '0'
    sll     $t0, $t0, 8
...
a6:     addi    $t0, $t0, '0'
    ...
    mtc1    $t0, $f17                   # Pega valor truncado do coprocessador 1
    cvt.s.w $f17, $f17                  # torna valor em float
    sub.s   $f10, $f10, $f17            # Subtrai do valor da mantissa, deixando o nÃƒÂºmero como 0.FXXXXX
...
OUT_LOOP_DEPOIS_DA_VIRGULA:    li      $t0, 'E'
    sll     $t0, $t0, 8
    or      $s5, $s5, $t0               # second byte saved
    srl     $t0, $t0, 8
...
EXPOENTE_CHAR1:    li      $t1, '+'
    sll     $t1, $t1, 16
    or      $s5, $s5, $t1               # third byte saved
    srl     $t1, $t1, 16

EXPOENTE_CHAR:     li      $t1, 10
    slti    $t5, $s4, 0
    beqz    $t5, EXPONENT
    not     $s4, $s4
    addi    $s4, $s4, 1
...
INFINITYPLUS:     li      $s5, 0x666E4920             # " Inf"
    sw      $s5, 0($s6)
...
INFINITYMINUS:     li      $s5, 0x666E492D             # " Inf"
    sw      $s5, 0($s6)
    li      $s5, 0x74696E69             # "init"
    sw      $s5, 4($s6)
    li      $s5, 0x00000079             # "y\0"
...
OBTAIN_MANTISSA_EXP:     mfc1    $s0, $f12                   # Take number in IEEE 754
# -------------------------
...
maiorQueZero:     cvt.w.s $f4, $f4                    # se x > 0, ceil(f) = trunca(x)
    j       salvaFloor

menorQueZero:     li      $t0, 0xBF79999A             # se x < 0, floor(f) = trunca(x-0.975)
    mtc1    $t0, $f23                   # f23 = 0.975
    add.s   $f4, $f4, $f23
    cvt.w.s $f4, $f4

salvaFloor:     mfc1    $s4, $f4                    # save ceil(f)
    cvt.s.w $f4, $f4
...
readFloat:     la      $a0, FloatBuffer            # buffer caracteres
...
floop1:     addi    $s1, $s1, 1                 # avanca o ponteiro da pilha
...
floop3:     sub     $t0, $s7, $s1               # numero total de casas decimais da parte inteira
    addi    $t0, $t0, -1
    move    $t7, $t0
...
endlp2:     addi    $s1, $s1, 1                 # avanca o ponteiro da pilha
...
stepE1:     addi    $s1, $s1, 1
    ...
    beq     $t0, '+', stepE2
    bne     $t0, '-', stepEE            # '-' = 45 Se for negativo prossegue, se nÃ£o ir para loop1
    li      $s2, 1                      # $s2 sinalizado 1 como negativo

stepE2:     addi    $s1, $s1, 1                 # avanca o ponteiro da pilha
...
stepE4:     sub     $t0, $s7, $s1               # numero total de casas decimais depois de E
    addi    $t0, $t0, -1
    mtc1    $t0, $f30
    move    $t7, $t0
    move    $s5, $s2
...
final:     mtc1    $s4, $f10                   # s4 is signal, then f10 will be signal
    cvt.s.w $f10, $f10
    bnez    $s4, sign                   # change signal if s4 != 0
        #Exponente
go_on1:     mtc1    $s2, $f14                   # s2 is signal of exponent, then f10 will be signal
    cvt.s.w $f14, $f14
    bnez    $s2, signE                  # change signal if s4 != 0

go_on2:     cvt.w.s $f13, $f13
    mfc1    $t7, $f13
    cvt.w.s $f13, $f13
    move    $t8, $ra
    jal     n10
...
print:     mov.s   $f0, $f1 #mov.s $f12, $f1
    #li $v0, 2
    #syscall
    j       backReadFloat
...
sign:     li      $t0, 0xBF800000 #-1
    mtc1    $t0, $f10
    mul.s   $f1, $f1, $f10
    j       go_on1

signE:     j       go_on2                      # isso nao e mais necessario... so volte

#######################-----------------------------#######################
#######################-----Calcula 10**n-----#######################
#######################-----------------------------#######################

n10:     addi    $sp, $sp, -20               # Argumento esta em t7 = n, saida em 10**n = $f31
    sw      $ra, 0($sp)
    sw      $t0, 4($sp)
    sw      $t7, 8($sp)
    swc1    $f0, 12($sp)
...
multiply_pos:     beqz    $t7, END2                   # a0 e o valor de n, vai subtraindo ate chegar em 0 ai vai para fim
    mul.s   $f31, $f31, $f0
    addi    $t7, $t7, -1
    j       multiply_pos

multiply_neg:     beqz    $t7, invert                 # a0 e o valor de n, vai subtraindo ate chegar em 0 ai vai para fim
    mul.s   $f31, $f31, $f0
    addi    $t7, $t7, -1
    j       multiply_neg

invert:     li      $t0, 0x3F800000             # 1
    mtc1    $t0, $f15
...
sdRead:     la      $s0, SD_INTERFACE_ADDR
...
sdDataReady:	lw      $t2, 0($s2)                     # Le word do buffer
 ...
 sdFim:	li      $v0, 0                          # Sucesso na transferencia.         NOTE: Hardcoded. Um teste de falha deve ser implementado.
    jr      $ra
############################################
